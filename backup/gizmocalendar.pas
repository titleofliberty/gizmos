...
begin
  ...
  for y := 0 to 7 do
    for x := 0 to 6 do
    begin
      grid[x,y] := TRect.Create(x * colw, y * rowh, (x * colw) + colw, (y * rowh) + rowh);
  ...
  mth := TRect.Create(grid[0,0].TopLeft, grid[1,0].BottomRight);

  Self.Canvas.TextRect(mth, 0, 0, FormatDateTime('mmmm', FDate), ts);

  Self.Canvas.TextRect(grid[0,1], 0, 0, 'Sun', ts);
  Self.Canvas.TextRect(grid[1,1], 0, 0, 'Mon', ts);
  Self.Canvas.TextRect(grid[2,1], 0, 0, 'Tue', ts);
  Self.Canvas.TextRect(grid[3,1], 0, 0, 'Wed', ts);
  Self.Canvas.TextRect(grid[4,1], 0, 0, 'Thu', ts);
  Self.Canvas.TextRect(grid[5,1], 0, 0, 'Fri', ts);
  Self.Canvas.TextRect(grid[6,1], 0, 0, 'Sat', ts);
  ...
  for y := 2 to 7 do
    ...
    begin
      ...
      Self.Canvas.TextRect(grid[x,y], 0, 0, FormatDateTime('d', stepdate), ts);
