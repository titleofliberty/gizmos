...
begin
  ...
  RegisterComponents('Gizmos',[TSimpleCard]);
end;
...
constructor TSimpleCard.Create(TheOwner: TComponent);
begin
...
procedure TSimpleCard.SetBody(AValue: TCardLabel);
begin
  if FBody = AValue then Exit;
  FBody := AValue;
end;

procedure TSimpleCard.SetFooter(AValue: TCardLabel);
begin
  if FFooter = AValue then Exit;
  FFooter := AValue;
end;

procedure TSimpleCard.SetHeader(AValue: TCardLabel);
begin
  if FHeader = AValue then Exit;
  FHeader := AValue;
end;

procedure TSimpleCard.SetPadding(AValue: Integer);
begin
...
procedure TSimpleCard.ChildChanged(Sender: TObject);
begin
  Invalidate;
end;
